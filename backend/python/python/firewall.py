...
#  Fichier : firewall.py
#  Description : Module de gestion du pare-feu logiciel.
#                Permet de bloquer/débloquer des adresses IP suspectes,
#                d'automatiser la défense contre les attaques, et de conserver
#                un historique sécurisé des actions.
#  --------------------------------------------------------------------------
#  Ce fichier est massivement commenté pour la pédagogie et la présentation.
#  Il explique la logique de gestion du pare-feu, la sécurité, et les choix techniques.
...
# Explications :
# -------------
# - La classe FirewallManager centralise toutes les opérations de sécurité réseau.
# - Les IPs bloquées sont enregistrées dans une base SQLite pour la traçabilité.
# - Les méthodes permettent de bloquer, débloquer, et nettoyer automatiquement les règles.
# - L'intégration avec iptables (Linux) permet une défense efficace et automatisée.
# - Les logs et exceptions sont gérés pour garantir la robustesse et l'auditabilité.
#
# Bonnes pratiques :
# - Ne jamais bloquer une IP de la whitelist.
# - Vérifier régulièrement la base de données pour éviter les blocages abusifs.
# - Sauvegarder/restaurer les règles iptables pour la persistance après redémarrage.
